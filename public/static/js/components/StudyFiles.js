...
const __component = {
  ...
  methods: {
    ...
    async saveText() {
      const name = this.newName.trim() || "study-note.txt"; const blob = new Blob([this.newContent], { type: "text/plain" }); await window.FlxonOS.db.put("files", { id: "file-" + Date.now(), name, type: blob.type, size: blob.size, blob, updatedAt: new Date().toISOString() }); this.newContent = ""; await this.refresh();
    },
    async download(id) {
  ...
  template: `<section class="window stack study-files"><div class="editor-heading"><div><h2>Student file system</h2><p class="muted">A private folder in this browser for notes, code, datasets, and project files.</p></div><button @click="refresh">Refresh</button></div><div class="row"><label class="button"><input type="file" multiple="" hidden="" @change="importFiles">Add files</label><button @click="saveText">Save text file</button><input placeholder="new-file.txt" data-teloce-model="newName"></div><textarea class="code file-editor" placeholder="Write a text file for your study workspace..." data-teloce-model="newContent"></textarea><div class="file-list"><if condition="!files.length"><div class="empty-state">No files yet. Add a file or save your first text note.</div></if><for key="file.id" item="file" in="files"><article data-teloce-key="{{ file.id }}" class="file-item"><div><strong>{{ file.name }}</strong><span class="muted">{{ file.size }}bytes · {{ file.type || 'text/plain' }}</span></div><div class="row file-actions"><button @click="download(file.id)">Download</button><button @click="remove(file.id)">Delete</button></div></article></for></div><p class="muted">Files stay in IndexedDB and are included in workspace backups. There is no account or server upload.</p></section>`,
};
...
//# sourceMappingURL=StudyFiles.js.map