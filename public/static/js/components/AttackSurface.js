...
const __component = {
  ...
  data() {
    return { url: "https://example.com", result: null, error: "", message: "Only public HTTP(S) same-origin links are included." };
  },

  methods: {
    async scan() {
      this.error = ""; this.result = null; this.message = "Mapping..."; try { const response = await fetch("/api/scanner/attack-surface", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: this.url }) }); const data = await response.json(); if (data.error) throw new Error(data.error); this.result = data; this.message = "Evidence collected. This is not an exploitation verdict."; } catch (error) { this.error = error.message; this.message = ""; }
    },
    downloadReport() {
      const blob = new Blob([JSON.stringify(this.result, null, 2)], { type: "application/json" }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = "flaxon-attack-surface.json"; link.click(); URL.revokeObjectURL(link.href);
    },
  },

  template: `<section class="window stack attack-surface"><div class="editor-heading"><div><h2>Attack surface</h2><p class="muted">Same-origin public link inventory for authorized review.</p></div><div class="row"><button @click="scan">Map surface</button><if condition="result"><button @click="downloadReport">Export JSON</button></if></div></div><div class="row"><input placeholder="https://example.com" data-teloce-model="url"></div><if condition="error"><p class="danger">{{ error }}</p></if><if condition="result"><div class="surface-summary"><span>{{ result.host }}</span><span>{{ result.pages }}page(s)</span><span>{{ result.links.length }}link(s)</span></div></if><if condition="result"><ul><for key="link" item="link" in="result.links"><li data-teloce-key="{{ link }}"><code>{{ link }}</code></li></for></ul></if><p class="muted">{{ message }}</p></section>`,
};
...
const __template = "<section class=\"window stack attack-surface\"><div class=\"editor-heading\"><div><h2>Attack surface</h2><p class=\"muted\">Same-origin public link inventory for authorized review.</p></div><div class=\"row\"><button @click=\"scan\">Map surface</button><if condition=\"result\"><button @click=\"downloadReport\">Export JSON</button></if></div></div><div class=\"row\"><input placeholder=\"https://example.com\" data-teloce-model=\"url\"></div><if condition=\"error\"><p class=\"danger\">{{ error }}</p></if><if condition=\"result\"><div class=\"surface-summary\"><span>{{ result.host }}</span><span>{{ result.pages }}page(s)</span><span>{{ result.links.length }}link(s)</span></div></if><if condition=\"result\"><ul><for key=\"link\" item=\"link\" in=\"result.links\"><li data-teloce-key=\"{{ link }}\"><code>{{ link }}</code></li></for></ul></if><p class=\"muted\">{{ message }}</p></section>";
const __style = "";
...
const __initialData = { url: "https://example.com", result: null, error: "", message: "Only public HTTP(S) same-origin links are included." };
...
const __installStyle = () => {
  ...
  const styleId = "teloce-style-c6222bcef";
...
//# sourceMappingURL=AttackSurface.js.map