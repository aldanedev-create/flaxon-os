...
const __component = {
  ...
  methods: {
    async scan() {
      this.error = ""; this.result = null; this.message = "Mapping..."; try { const response = await fetch("/api/scanner/attack-surface?url=" + encodeURIComponent(this.url), { cache: "no-store" }); const data = await response.json(); if (data.error) throw new Error(data.error); this.result = data; this.message = "Evidence collected. This is not an exploitation verdict."; } catch (error) { this.error = error.message; this.message = ""; }
    },
    downloadReport() {
