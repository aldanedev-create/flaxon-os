...
const __component = {
  ...
  data() {
    return { items: [], message: "Media stays on this device." };
  },

  methods: {
    async refresh() {
      this.items = await window.FlxonOS.db.list("media");
    },
    formatSize(size) {
      const value = Number(size || 0); return value < 1048576 ? Math.round(value / 1024) + " KB" : (value / 1048576).toFixed(1) + " MB";
    },
    async remove(id) {
      await window.FlxonOS.db.delete("media", id); this.message = "Asset removed locally."; await this.refresh();
    },
  },

  mounted() {
    this.refresh(); window.addEventListener("flaxon:media-changed", this.refresh);
  },
  updated() {
  ...
  beforeUnmount() {
    window.removeEventListener("flaxon:media-changed", this.refresh);
  },

  template: `<section class="window stack media-library"><div class="editor-heading"><div><h2>Media library</h2><p class="muted">Local media assets used by Flaxon Studio.</p></div><button @click="refresh">Refresh</button></div><div class="media-list"></div><p class="muted">{{ message }}</p></section>`,
};
...
const __template = "<section class=\"window stack media-library\"><div class=\"editor-heading\"><div><h2>Media library</h2><p class=\"muted\">Local media assets used by Flaxon Studio.</p></div><button @click=\"refresh\">Refresh</button></div><div class=\"media-list\"></div><p class=\"muted\">{{ message }}</p></section>";
const __style = "";
...
const __initialData = { items: [], message: "Media stays on this device." };
...
const __installStyle = () => {
  ...
  const styleId = "teloce-style-55b282cbc";
