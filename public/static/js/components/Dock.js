...
const __component = {
  ...
  methods: {
    go(view) {
      window.dispatchEvent(new CustomEvent("flaxon:navigate", { detail: view }));
    },
    start() {
      window.dispatchEvent(new CustomEvent("flaxon:start"));
    },
  },

  template: `<div class="os-dock"><button class="dock-item start-button" aria-label="Start" @click="start()"><span class="start-mark">F</span><span>Launcher</span></button><button class="dock-item" @click="go('home')">Home</button><button class="dock-item" @click="go('network')">Pulse</button><button class="dock-item" @click="go('video')">Studio</button><button class="dock-item" @click="go('study')">Study</button></div>`,
};
...
const __template = "<div class=\"os-dock\"><button class=\"dock-item start-button\" aria-label=\"Start\" @click=\"start()\"><span class=\"start-mark\">F</span><span>Launcher</span></button><button class=\"dock-item\" @click=\"go('home')\">Home</button><button class=\"dock-item\" @click=\"go('network')\">Pulse</button><button class=\"dock-item\" @click=\"go('video')\">Studio</button><button class=\"dock-item\" @click=\"go('study')\">Study</button></div>";
const __style = "";
...
const __installStyle = () => {
  ...
  const styleId = "teloce-style-035213f31";
