...
const __component = {
  ...
  mounted() {
    statusTimer = setInterval(() => this.tick(), 1000);
  },
  beforeUnmount() {
    clearInterval(statusTimer); statusTimer = null;
  },

  template: `<footer class="status-bar"><span>Flaxon OS</span><span>{{ message }}</span><span>{{ clock }}</span></footer>`,
};
...
const __template = "<footer class=\"status-bar\"><span>Flaxon OS</span><span>{{ message }}</span><span>{{ clock }}</span></footer>";
const __style = "";
...
const __initialData = { message: "Ready", clock: "" };
...
const __installStyle = () => {
  ...
  const styleId = "teloce-style-f93e1d0ca";
