...
const __component = {
  ...
  methods: {
    ...
    setModuleVisibility(view) {
      const visible = { "python-module": view === "playground", "surface-module": view === "scanner", "files-module": view === "study", "media-module": view === "video", "video-module": view === "video", "workspace-module": false }; for (const [id, show] of Object.entries(visible)) { const host = document.getElementById(id); if (host) host.hidden = !show; } const content = document.querySelector(".content"); if (content) content.hidden = ["scanner", "playground", "study", "video"].includes(view);
    },
    open(view) {
      this.windowMinimized = false; document.querySelector(".workspace")?.classList.remove("window-minimized"); this.view = view; this.title = view.charAt(0).toUpperCase() + view.slice(1); this.setModuleVisibility(view); window.FlxonOS.windows.open(view); this.metrics.windows = window.FlxonOS.windows.list().length;
    },
    minimizeWindow() {
    ...
    async scan() {
      this.scanError = ""; this.scanResult = null; try { const response = await fetch("/api/scanner/check", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: this.scanUrl, crawl: true }) }); const data = await response.json(); if (data.error) throw new Error(data.error); this.scanResult = data; } catch (error) { this.scanError = error.message; }
    },
    async checkPhishing() {
      try { const response = await fetch("/api/checkers/phishing", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: this.scanUrl }) }); const data = await response.json(); if (data.error) throw new Error(data.error); this.phishingResult = data.risk + (data.reasons && data.reasons.length ? ": " + data.reasons.join(" ") : ""); } catch (error) { this.phishingResult = error.message; }
    },
    async checkPython() {
      try { const response = await fetch("/api/playground/python/check", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source: this.pythonSource }) }); const data = await response.json(); this.pythonResult = data.ok ? data.message : data.error; } catch (error) { this.pythonResult = error.message; }
    },
    preview() {
